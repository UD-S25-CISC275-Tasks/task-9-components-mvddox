...
export function isValid(question: Question, answer: string): boolean {
    if (question.type === "short_answer_question") {
        ...
        question.options.find((option: string): boolean => answer === option)
    ) {
        return true;
    }
    return false;
...
export function toMarkdown(question: Question): string {
    ...
    if (question.type === "multiple_choice_question") {
        MCQ = question.options.reduce(
            (choice: string, curr: string) => choice + "- " + curr + "\n",
            "",
        );
        MCQ = MCQ.slice(0, MCQ.length - 1);
    }
    let finalQuestion: string = "# " + question.name + "\n" + question.body;
